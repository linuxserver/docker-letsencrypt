...
docker create \
  ...
  -e CERT_PASSWORD=crypticpassword `#optional` \
  -p 443:443 \
  -p 80:80 `#optional` \
...
services:
  letsencrypt:
    ...
    environment:
	  ...
	  - CERT_PASSWORD=crypticpassword #optional
    volumes:
...
| `-e CERT_PASSWORD=crypticpassword` | Cryptic password used for creating the `privkey.pfx` certificate. Note: `crypticpassword` is used as a stand-in for a password of your own choosing. |
...
* This container includes auto-generated pfx and private-fullchain-bundle pem certs that are needed by other apps like Emby and Znc.
  ...
  * These certs include:
  ...
  2. `privkey.pfx`, a format supported by Microsoft and commonly used by dotnet apps such as Emby Server. The shared certificate (located at `/config/keys/letsencrypt/`) has no password. The certificate, which is generated for your site (located at `/config/letsencrypt/etc/letsencrypt/live/<your.domain.url>/`), tha password is what you set for `CERT_PASSWORD`.
